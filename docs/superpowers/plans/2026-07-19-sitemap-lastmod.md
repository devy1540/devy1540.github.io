...
**목표:** 기존 게시글은 `date`를 최종 수정일로 간주하고, 선택적인 `updated`가 있는 글은 sitemap·JSON-LD·화면에 일관되게 반영한다. 신뢰할 수 없는 고정/집계 페이지 빌드 날짜는 sitemap에서 제거한다.
...
1. `updated ?? date` 선택, 과거 `updated`, 미래 `updated`, 잘못된 날짜 형식을 검증하는 테스트를 먼저 작성한다.
...
1. `PostMeta`와 빌드용 게시글 타입에 선택적 `updated`를 추가한다.
...
2. 게시글 URL은 `updated ?? date`를 출력한다.
...
1. 게시글 JSON-LD의 `dateModified`를 `updated ?? date`로 출력한다.
