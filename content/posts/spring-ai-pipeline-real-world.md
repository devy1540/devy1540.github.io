...
date: "2026-03-06"
description: "Python MVP를 Spring AI 기반 7단계 파이프라인으로 재설계한 실전 경험을 공유합니다."
...
우리 서비스에는 사용자의 음성 데이터를 AI로 분석해서 진단 리포트를 생성하는 기능이 있다. 초기에는 Python으로 만든 MVP가 돌아가고 있었는데, 구조가 단순했다.

- 하나의 프롬프트에 모든 걸 담아서 LLM에 던지는 방식
- 결과물의 품질이 불안정 — 할루시네이션, 포맷 깨짐, 누락 등
- CS 인입이 꾸준히 발생

무엇보다 기존 백엔드가 전부 Java/Spring 기반이라 Python 서비스 하나를 별도로 운영하는 것 자체가 부담이었다. 배포 파이프라인도 따로, 모니터링도 따로, 온콜 대응도 따로. 서비스가 커질수록 이 간극이 점점 벌어졌다.

## 왜 Spring AI인가

LLM 파이프라인을 Java로 가져오기로 결정한 뒤, 선택지를 검토했다.

1. **직접 HTTP 클라이언트로 호출** — 가장 단순하지만 모델별 API 차이를 전부 직접 대응해야 한다
...
무엇보다 Gemini, OpenAI, Amazon Bedrock 같은 서로 다른 LLM 프로바이더를 `ChatModel`이라는 하나의 인터페이스로 추상화해준다는 점이 결정적이었다. 프로바이더마다 다른 API 스펙, 인증 방식, 요청/응답 포맷을 각각 대응할 필요 없이, 동일한 `ChatClient` 코드로 어떤 모델이든 호출할 수 있다. 덕분에 스텝별로 다른 모델을 배치하더라도 코드의 일관성이 유지되고, 모델 교체가 설정 변경만으로 가능했다.
...
여기서도 블록 인덱스 패턴을 활용한다. LLM은 **변경이 필요한 블록만** 인덱스와 보정 텍스트를 반환하고, 변경이 없는 블록은 아예 응답에 포함하지 않는다. 전체 블록 중 수정이 필요한 건 일부에 불과하기 때문에, 이 diff-only 방식으로 출력 토큰을 크게 줄일 수 있다.
...
Spring AI의 장점은 여기서 빛난다. `ChatModel` 인터페이스가 통일되어 있어서 모델 교체가 설정 변경만으로 가능하다.
...
각 스텝에서 사용할 `ChatClient`를 Bean으로 분리 등록하고, 스텝 구현체에 주입하는 방식으로 깔끔하게 분리했다.
...
- Spring Boot 자동설정, 프로퍼티 바인딩 등 익숙한 패턴으로 개발할 수 있다

**아쉬웠던 점:**
- 아직 1.0 이전 버전이라 마이너 업데이트에서 breaking change가 종종 있었다
- 일부 모델 프로바이더(특히 Bedrock)의 지원이 완벽하지 않아 직접 커스터마이징이 필요한 부분이 있었다

![진단 파이프라인 배포 완료 슬랙 알림](/images/posts/spring-ai-pipeline-deploy.png)

단일 프롬프트로 모든 걸 해결하려는 접근에서 벗어나, **작업을 명확히 분리하고 각 스텝에 최적화된 모델을 배치하는 전략**이 품질과 비용 모두에서 효과적이었다. LLM을 프로덕션에 적용할 때 "어떤 모델을 쓸까"보다 "어떻게 파이프라인을 설계할까"가 더 중요한 질문이라는 걸 다시 한 번 확인한 프로젝트였다.
