...
핵심은 **MDC와 OTel Span을 동시에 설정**하는 점이다:
...
이 구조의 장점은 **모니터링 설정도 GitOps로 관리**된다는 점이다. 노이즈 필터에 새 패턴을 추가하면 PR → 리뷰 → 머지 → ArgoCD 자동 배포로 반영된다. 콘솔에서 설정을 바꾸고 문서화를 깜빡하는 일이 없다.
...
OTel Java Agent가 HTTP 요청의 헤더를 전부 span attribute로 기록하는데, 여기에 `Authorization` 헤더도 포함되어 있었다. 그 결과 Tempo에 JWT 토큰이 그대로 저장됐다.
...
물론 트레이드오프는 있다. Datadog은 에이전트 하나만 붙이면 모든 게 자동으로 돌아가는데, LGTM 스택은 직접 운영한다. Helm 업그레이드, 디스크 관리, S3 비용 모니터링 등이 추가 업무로 생긴다. 전담 인프라 팀이 없는 상황에서는 부담이 될 수 있다.
