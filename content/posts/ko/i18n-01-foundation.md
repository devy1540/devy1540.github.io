...
문제는 키가 수백, 수천 개로 늘면 **로케일 간 키 누락**이 반드시 생긴다는 점이다. `ko.json`에만 있고 `ja.json`엔 빠진 키가 있으면 그 자리에 한국어가 튀어나오거나 키 문자열이 그대로 노출된다. 그래서 세 로케일의 키 집합이 어긋나면 걸러내는 **검증 스크립트**를 CI에 붙였다.
