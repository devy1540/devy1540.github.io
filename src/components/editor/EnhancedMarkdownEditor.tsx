import { FC, useCallback, useEffect, useRef, useMemo, memo, forwardRef } from 'react';
...
interface EnhancedMarkdownEditorProps {
  ...
  placeholder?: string;
  autoFocus?: boolean;
  onToolbarAction?: (action: string, value?: any) => void;
}

export const EnhancedMarkdownEditor = memo(forwardRef<HTMLTextAreaElement, EnhancedMarkdownEditorProps>(({
  className,
  ...
  const insertTextAtCursor = useCallback((before: string, after: string = '') => {
    ...
    if (!((globalThis as any).__editorTimeouts)) {
      (globalThis as any).__editorTimeouts = [];
    }
    (globalThis as any).__editorTimeouts.push(timeoutId);
  ...
  }, [insertTextAtCursor, onToolbarAction]);
  ...
  useEffect(() => {
    return () => {
      ...
      const timeouts = (globalThis as any).__editorTimeouts || [];
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development' && content.length > 10000) {
      console.log(`[Enhanced Editor] Large content detected: ${content.length} characters`);
    }
  }, [content.length]);
  ...
  return (
    ...
    >
      ...
      >
        Markdown editor. Use keyboard shortcuts: Ctrl+B for bold, Ctrl+I for italic, Ctrl+K for links.
        Use Tab for indentation. Navigate with arrow keys and standard text editing commands.
      </div>
    </div>
  );
}));
